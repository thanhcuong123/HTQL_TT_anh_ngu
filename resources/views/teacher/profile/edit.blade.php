...
<div class="update-card">
    <div class="update-header flex flex-col md:flex-row items-center justify-center md:justify-start">
        <img id="imagePreview" class="profile-image-preview mr-0 md:mr-8"
            src="{{ $giaoVien->hinhanh ? asset('storage/' . $giaoVien->hinhanh) : 'https://placehold.co/120x120/a7f3d0/10b981?text=Avatar' }}"
            alt="Ảnh đại diện">
        <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            <!-- <div class="form-group">
                ...
                <input type="text" id="ten" name="ten"
                    ...
                    value="{{ old('ten', $giaoVien->ten) }}" required>
                @error('ten')
            ...
            <div class="form-group">
                ...
                <input type="text" id="diachi" name="diachi"
                    ...
                    value="{{ old('diachi', $giaoVien->diachi) }}">
                @error('diachi')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="date" id="ngaysinh" name="ngaysinh"
                    ...
                    value="{{ old('ngaysinh', $giaoVien->ngaysinh ? \Carbon\Carbon::parse($giaoVien->ngaysinh)->format('Y-m-d') : '') }}">
                @error('ngaysinh')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="gioitinh" name="gioitinh"
                    ...
                    <option value="Nam" {{ old('gioitinh', $giaoVien->gioitinh) == 'Nam' ? 'selected' : '' }}>Nam</option>
                    <option value="Nữ" {{ old('gioitinh', $giaoVien->gioitinh) == 'Nữ' ? 'selected' : '' }}>Nữ</option>
                    <option value="Khác" {{ old('gioitinh', $giaoVien->gioitinh) == 'Khác' ? 'selected' : '' }}>Khác</option>
                </select>
                @error('gioitinh')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            ...
            {{-- Dropdown cho Chuyên môn --}}
            <div class="form-group">
                <label for="chuyenmon_id" class="form-label">Chuyên môn:</label>
                <select id="chuyenmon_id" name="chuyenmon_id"
                    class="form-input @error('chuyenmon_id') is-invalid @enderror">
                    <option value="">Chọn chuyên môn</option>
                    @foreach($chuyenMons as $chuyenMon)
                    <option value="{{ $chuyenMon->id }}" {{ old('chuyenmon_id', optional($giaoVien->chuyenMon)->id) == $chuyenMon->id ? 'selected' : '' }}>
            ...
            {{-- Dropdown cho Chức danh --}}
            <div class="form-group">
                <label for="chucdanh_id" class="form-label">Chức danh:</label>
                <select id="chucdanh_id" name="chucdanh_id"
                    class="form-input @error('chucdanh_id') is-invalid @enderror">
                    <option value="">Chọn chức danh</option>
                    @foreach($chucDanhs as $chucDanh)
                    <option value="{{ $chucDanh->id }}" {{ old('chucdanh_id', optional($giaoVien->chucDanh)->id) == $chucDanh->id ? 'selected' : '' }}>
                        {{ $chucDanh->ten }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select id="hocvi_id" name="hocvi_id"
                    class="form-input @error('hocvi_id') is-invalid @enderror">
                    <option value="">Chọn học vị</option>
                    @foreach($hocVis as $hocVi)
                    <option value="{{ $hocVi->id }}" {{ old('hocvi_id', optional($giaoVien->hocVi)->id) == $hocVi->id ? 'selected' : '' }}>
                        {{ $hocVi->tenhocvi }}
                    </option>
                    @endforeach
                </select>
            ...
            {{-- <div class="form-group md:col-span-2">
                ...
                <input type="email" id="email" name="email"
                       ...
                       value="{{ old('email', $giaoVien->user->email ?? '') }}" required>
            @error('email')
            <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div> --}}
...
<div class="mt-8 flex justify-end space-x-4">
    <a href="{{ route('teacher.profile') }}" class="btn-secondary inline-flex items-center">
