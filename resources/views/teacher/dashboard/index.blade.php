...
<div class="teacher-dashboard-card">
    <div class="teacher-info">
        <h3>Chào mừng, {{ $giaoVien->ten ?? 'Giáo viên' }}!</h3>
        <p><i class="fas fa-id-card"></i> Mã giáo viên: <strong>{{ $giaoVien->magiaovien ?? 'N/A' }}</strong></p>
        <p><i class="fas fa-envelope"></i> Email: <strong>{{ $giaoVien->user->email ?? 'N/A' }}</strong></p>
        <p><i class="fas fa-phone"></i> Điện thoại: <strong>{{ $giaoVien->sdt ?? 'N/A' }}</strong></p>
    </div>

    <div class="schedule-section">
        <h4><i class="fas fa-calendar-day"></i> Lịch dạy hôm nay ({{ \Carbon\Carbon::parse($today)->format('d/m/Y') }})</h4>

        @if($scheduleToday->isEmpty())
        <div class="no-schedule">
            <p>Hôm nay ({{ \Carbon\Carbon::parse($today)->format('d/m/Y') }}) bạn không có lịch dạy nào.</p>
