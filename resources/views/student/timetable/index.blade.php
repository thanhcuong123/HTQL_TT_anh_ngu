...
<div class="timetable-card">
    ...
    {{-- Hiển thị thông báo nếu học viên không có lớp nào --}}
    @if($hocvien->lophocs->isEmpty())
    <div class="no-schedule-message">
        <i class="fas fa-exclamation-circle"></i>
        <p>Bạn hiện chưa đăng ký lớp học nào, do đó không có thời khóa biểu để hiển thị.</p>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Chỉ khởi tạo lịch nếu có phần tử lịch (tức là học viên có lớp học)
        if (calendarEl) {
            const calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events: {
                    url: '{{ route("student.timetable.events") }}', // API endpoint để lấy dữ liệu sự kiện
                    method: 'GET', // Phương thức HTTP
                    failure: function(error) {
                        console.error('Lỗi khi tải sự kiện lịch:', error);
                        // Thay alert bằng một thông báo thân thiện hơn trên UI
