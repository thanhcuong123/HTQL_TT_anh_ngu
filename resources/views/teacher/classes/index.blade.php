...
<title>Các Lớp Dược Phân Công Dạy</title>
@endsection
...
<div class="card">
    <div class="card-body">
        <h3>Các Lớp Dược Phân Công Dạy</h3>

        @if($assignedClasses->isEmpty())
        <div class="no-classes-message">
            <i class="fas fa-exclamation-circle"></i>
            <p>Hiện tại bạn chưa được phân công dạy lớp học nào.</p>
        ...
        <div class="class-card-container">
            ...
            <div class="class-card">
                <div>
                    ...
                    <p><i class="fas fa-users"></i> Sĩ số: <strong>{{ $class->hocviens_count ?? 0 }} / {{ $class->soluonghocvientoida ?? 'N/A' }}</strong></p>
                    <span class="status status-{{ $class->trangthai }}">{{ str_replace('_', ' ', ucfirst($class->trangthai)) }}</span>
                </div>
                <div class="card-footer">
                    {{-- Giả định có route chi tiết lớp học cho giáo viên --}}
                    <a href="{{ route('teacher.classes.show', $class->id) }}">Xem chi tiết <i class="fas fa-arrow-right"></i></a>
                </div>
            </div>
            @endforeach
        </div>
