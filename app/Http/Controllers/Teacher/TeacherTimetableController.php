...
{
    /**
     * Hiển thị trang thời khóa biểu dạng lịch cho giáo viên.
     *
    ...
    public function index()
    {
        ...
        $user = Auth::user();
        $giaoVien = $user->giaovien; // Giả định User model có mối quan hệ 'giaovien'
        ...
        return view('teacher.timetable.index', compact('giaoVien'));
    }
    ...
    public function getCalendarEvents(Request $request)
    {
        try {
            // Kiểm tra xem người dùng đã đăng nhập chưa
            if (!Auth::check()) {
                Log::warning('Unauthorized access to teacher calendar events API.');
                return response()->json(['error' => 'Unauthorized'], 401);
            }

            // Lấy thông tin giáo viên của người dùng đang đăng nhập
            $user = Auth::user();
            $giaoVien = $user->giaovien; // Lấy mối quan hệ giaovien từ đối tượng User

            if (!$giaoVien) {
                Log::warning('Authenticated user has no associated GiaoVien profile for calendar events API.');
                return response()->json(['error' => 'Tài khoản của bạn không liên kết với hồ sơ giáo viên.'], 403);
            }
            ...
            Log::debug("FullCalendar API Request for teacher ID {$giaoVien->id}: Start={$start->toDateString()}, End={$end->toDateString()}");
            ...
            // Lấy tất cả các thời khóa biểu cho giáo viên này trong khoảng thời gian yêu cầu
            ...
            Log::debug('Number of ThoiKhoaBieu items fetched for teacher: ' . $thoikhoabieuItems->count());
            // dd($thoikhoabieuItems->toArray()); // Uncomment for deep debugging
            ...
            foreach ($thoikhoabieuItems as $item) {
                ...
                if (!$item->lophoc || !$item->phonghoc || !$item->thu || !$item->cahoc || !$item->kynang) {
                    Log::warning("Mục thời khóa biểu ID " . ($item->id ?? 'N/A') . " thiếu dữ liệu mối quan hệ (lớp, phòng, thứ, ca, kỹ năng). Bỏ qua.");
                    continue;
                }
                ...
                // Xác định khoảng thời gian hiệu lực của lịch dạy này trong phạm vi yêu cầu của FullCalendar
                ...
                for ($date = clone $actualLopHocStartDate; $date->lte($actualLopHocEndDate); $date->addDay()) {
                    // Nếu ngày hiện tại trong vòng lặp trùng với thứ của lịch dạy
                    if ($date->dayOfWeek == $targetDayOfWeek) {
                        ...
                        $events[] = [
                            ...
                            'url'   => route('teacher.classes.show', $lophoc->id), // Link đến trang chi tiết lớp học của giáo viên
                            'extendedProps' => [
                                'classId'   => $lophoc->id,
            ...
            Log::debug('Total events generated for teacher: ' . count($events));
            // dd($events); // Uncomment for deep debugging

            // Trả về dữ liệu sự kiện dưới dạng JSON
            return response()->json($events);
        } catch (\Exception $e) {
            Log::error("Lỗi khi lấy sự kiện lịch giáo viên: " . $e->getMessage() . " tại dòng " . $e->getLine() . " trong file " . $e->getFile());
